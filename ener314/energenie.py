...
def send_test():
    mode_energenie_transmit()
    time.sleep(1)

    # dev_id = 0x05b421    # 4 way extension
    # dev_id = 0x08a39c    # desk lamp
    dev_id = 0x07b2df  # desk spot light

    pkt = EnergeniePacket(dev_id, 0x0)
...
def main():
    #logger.info("Encode {}".format(encode_payload(0x8A39C1)))

    logger.info("Starting up Energnie")
    rfm69.initialize()
    listen_test()
    #send_test()
    rfm69.shutdown()
